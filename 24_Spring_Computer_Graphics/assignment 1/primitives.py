...
class Cylinder:
    ...
    def __init__(self, slices, height=1.0, radius=0.5):
        self.vertices = []
        self.indices = []
        self.colors = []
        ...
        for i in range(slices):
            ...
            self.vertices.extend([x, - height / 2, z])
            color = (int(math.cos(theta) * 255) + int(math.sin(theta) * 255)) // 2
            self.colors.extend([color, color, color, 255])
            self.colors.extend([color, color, color, 255])
        ...
        for i in range(slices):
            ...
            v2 = (i * 2 + 2) % (slices * 2)
            v3 = (i * 2 + 3) % (slices * 2)
            self.indices.extend([v1, v0, v2])
            self.indices.extend([v1, v2, v3])

        self.vertices.extend([0, height / 2, 0])
        self.colors.extend([200, 200, 200, 255])
        self.vertices.extend([0, - height / 2, 0])
        self.colors.extend([200, 200, 200, 255])
