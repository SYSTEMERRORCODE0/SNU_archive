...
class ID(Pipe):
    ...
    def __init__(self):
        ...
        #
        #   self.ID.c_br_miss       # Pipe.ID.c_br_miss
        #
        #   self.IF_stall           # Pipe.ID.IF_stall
        #   self.ID_stall           # Pipe.ID.ID_stall
        #   self.RR_stall           # Pipe.ID.RR_stall
...
class RR(Pipe):
    ...
    reg_c_op1_sel       = WORD(OP1_X)       # RR.reg_c_op1_sel
    reg_c_op2_sel       = WORD(OP2_X)       # RR.reg_c_op2_sel
    reg_c_rs1_oen       = WORD(OEN_0)       # RR.reg_c_rs1_oen
    reg_c_rs2_oen       = WORD(OEN_0)       # RR.reg_c_rs2_oen
    ...
    def __init__(self):
        ...
        #
        #   self.fwd_op1            # Pipe.RR.fwd_op1
        #   self.fwd_op2            # Pipe.RR.fwd_op2
        #   self.fwd_rs2            # Pipe.RR.fwd_rs2
        #
        #----------------------------------------------
    ...
    def compute(self):
        ...
        alu_op2 =   self.op2_data   if self.c_op2_sel == OP2_RS2 else \
                    self.imm        if self.c_op2_sel == OP2_IMI else \
                    self.imm        if self.c_op2_sel == OP2_IMS else \
                    self.imm        if self.c_op2_sel == OP2_IMB else \
                    self.imm        if self.c_op2_sel == OP2_IMU else \
                    self.imm        if self.c_op2_sel == OP2_IMJ else \
                    WORD(0)
...
class EX(Pipe):
    ...
    def compute(self):
        ...
        # Calculate the branch/jump target address using an adder (forwarded to ID)
